...
function CreateVirtualAccountModal({
  ...
  const { mutate: dynamicCreateVirtualAccount, isLoading: dynamicCreateVirtualAccountLoading } = useDynamicCreateVirtualAccount(gatewayId)

  const virtualAccountOptions = virtualAccountDetails && virtualAccountDetails?.data?.map(
  ...
  const accountFormDetails = virtualAccountDetails?.data?.filter((item: { optioncode: string; }) => {
...
const generateSchema = (field: any) => {
  if (field.type === 'text') {
    if (field.required){
      return z.string().min(1, { message: `Enter a value for ${field.label}` });
    }
    return z.string()
  } else if (field.type === 'number') {
    if (field.required){
      return z.number().min(1, { message: `Enter a value for ${field.label}` });
    }
   return z.number()
  } 
  return z.unknown();
...
const virtualAccountFormValidator = z.object(
  Object.fromEntries(
    (accountFormDetails && accountFormDetails[0]?.data?.map((field: { name: any; }) => [field.name, generateSchema(field)])) || []
  )
);
  ...
  const initialValues = generateInitialValues(accountFormDetails && accountFormDetails[0]?.data || []);
  ...
  if (virtualAccountDetailsError) {
    return showNotification({
      ...
      message: "Unable to create virtual account at the moment. Please try again later.",
      color: "red",
    });
}
        ...
        >
          ...
          <Select
            ...
            onChange={(value) => {
              ...
              addVirtualAccountForm.setFieldValue("title", value as string|number);
            }}
          />
          <Stack>
            {accountFormDetails && accountFormDetails[0]?.data?.map((item: any) => {
