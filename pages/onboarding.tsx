...
export default function Onboarding() {
    ...
    const {
        data: userSelectedGateways,
        isLoading: userSelectedGatewaysLoading,
    } = useGetSelectedGateways();
    ...
    const showIndividualProfileNextBtn = Boolean(
        data?.data?.client_type === CLIENT_TYPES.INDIVIDUAL &&
            !basicProfile?.data.business_name &&
            (basicProfile?.data.state || basicProfile?.data.city)
    );
            ...
            <section className="rounded-lg bg-gray-30 border text-gray-90 p-5 flex-grow">
                <Tabs
                    ...
                    <Tabs.List>
                        ...
                        <Tabs.Tab value="id-verification">
                            ID Verification
                        </Tabs.Tab>
                        {data?.data?.client_type === CLIENT_TYPES.CORPORATE && (
                            <Tabs.Tab value="document-upload">
                                Document Upload
                            </Tabs.Tab>
                        )}
