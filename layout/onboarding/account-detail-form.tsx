...
}: // gateway
{
  ...
  // console.log({ banks });
  const [defaultBank, setDefaultBank] = useState<SelectBankType>({ label: "", value: "" });
  ...
  const accountDetailForm = useForm({
    initialValues: {
      bank_name: formData?.bank_name || accountDeets?.data?.bank_name || "",
      bank: formData?.bank || "",
  ...
  const showNextBtn = Boolean(
    (accountDeets?.data?.account_name && accountDeets?.data?.account_number) ||
      (formData?.account_name && formData?.account_number && formData?.bank_name)
  );
  ...
  return (
    ...
    >
      ...
      <Select
        ...
        size="lg"
        // {...accountDetailForm.getInputProps("bank_name")}
        // value={defaultBank.label || accountDetailForm.values.bank_name}
        defaultValue={defaultBank?.label || accountDetailForm.values.bank_name}
